...
Uses data from CSV + database (purchases), filtered to last 365 days.
Run via Task Scheduler at 2am for automatic daily retraining.
"""
...
def main():
    ...
    for i, med in enumerate(medications, 1):
        ...
        try:
            # Prepare time series (last 365 days, merges CSV + DB purchase data)
            series, apply_log = prepare_time_series(df, med, lookback_days=365)
            
            if len(series) < 20:
                print(f"  [SKIP] Insufficient data (only {len(series)} points)")
                failed_count += 1
                continue
